...
func (d *DataFetchService) FetchAllGuildMessages(guildID string) ([]string, error) {
	...
	for _, channel := range guild.Channels {
		...
		go func(channel *discordgo.Channel) {
			...
			if err != nil {
				logger.Errorf("Failed to fetch messages for channel #%s: %v", channel.Name, err)
				return
			}
			messageCh <- messages
		}(channel)
	...
	logger.Infof("Fetched %d messages in guild %s", len(allMessages), guild.Name)
	...
	for len(messages) < d.MessageLimit {
		...
		if err != nil {
			...
			if errorCount > d.MaxFetchErrors {
				logger.Warnf("Error limit reached for channel #%s: %v", channel.Name, err)
				break
			}
			continue
		}
